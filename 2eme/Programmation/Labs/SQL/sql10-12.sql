...
SELECT Societe,C.CodeCli,Cmd.* FROM Client C, Commande Cmd WHERE C.CodeCli = Cmd.CodeCli;
SELECT  NomMess,Cmd.* FROM Messager M, Commande Cmd WHERE M.NoMess = Cmd.NoMess;
SELECT F.NoFour,P.* FROM Fournisseur F, Produit P WHERE F.NoFour = P.NoFour;
SELECT F.*,P.* FROM Fournisseur F, Produit P WHERE F.NoFour = P.NoFour GROUP BY F.Pays;
SELECT Count(Cmd.NoEmp) AS "Nbre Cmd",E.Nom,Cmd.* FROM Employe E, Commande Cmd WHERE E.NoEmp = Cmd.NoEmp GROUP BY Cmd.NoEmp;
SELECT Societe,Nom,Prenom,Client.Pays FROM Client,Employe WHERE Client.Pays = Employe.Pays
SELECT Count(P.CodeCateg),Societe,Cmd.* FROM Commande Cmd,Client,DetailCommande DCmd,Produit P WHERE Client.CodeCli = Cmd.CodeCli AND Cmd.NoCom = DCmd.Nocom
AND DCmd.Refprod = P.Refprod GROUP BY P.CodeCateg;
SELECT * FROM Employe WHERE NoEmp NOT IN (SELECT NoEmp FROM Commande);
SELECT Count(P.NoFour) AS "Nbre prod",P.* FROM Produit P WHERE NoFour IN (SELECT NoFour FROM Fournisseur WHERE NoFour = P.NoFour) GROUP BY NoFour;
SELECT Count(Cmd.NoEmp),Cmd.* 
    FROM Commande Cmd 
    WHERE NoEmp IN (SELECT NoEmp 
                        FROM Employe WHERE RendCompteA =(
                            SELECT NoEmp
                            FROM Employe
                            WHERE Nom = 'Emery' AND Prenom = 'Paul')) 
    GROUP BY NoEmp;
SELECT COUNT(*)
    FROM Commande
    INNER JOIN Employe ON Commande.NoEmp = Employe.NoEmp
    INNER JOIN Employe AS Responsable ON Employe.RendCompteA = Responsable.NoEmp
    WHERE Responsable.Nom = 'Emery' AND Responsable.Prenom = 'Patrick';
SELECT * 
    FROM Produit P 
    WHERE NOT EXISTS (SELECT *
                        FROM DetailCommande WHERE Refprod = P.Refprod);
SELECT *
    FROM Fournisseur F
    WHERE EXISTS (SELECT *
                        FROM Produit P
                        WHERE NoFour = F.NoFour AND EXISTS(SELECT * 
                            FROM DetailCommande DCmd 
                            WHERE Refprod = P.RefProd AND EXISTS(SELECT *
                                FROM Commande WHERE NoCom = DCmd.Nocom AND PaysLiv = 'France')))
    GROUP BY Pays;
SELECT * FROM Fournisseur F 
WHERE EXISTS(SELECT * FROM Produit
            WHERE NoFour = F.NoFour 
            AND QteParUnit LIKE '%canette%'
            OR QteParUnit LIKE '%bouteille%')
    GROUP BY F.Societe,NoFour;
SELECT * FROM Client
    WHERE NOT EXISTS (
        SELECT *
        FORM Produit
        WHERE
            NoFour = (
                SELECT NoFour
                FROM Fournisseur
                WHERE Societe = 'Exoctic Liquids')
            AND NOT EXISTS(
                SELECT *
                FROM DetailCommande
                INNER JOIN Commande ON Commande.NoCom = DetailCommande.Nocom
